...
void RGBupdate() {
  ...
  const float rScale = 0.0918;
  const float gScale = 0.1439;
  const float bScale = 0.0898;
  ...
  const float targetR[] = { 153, 117, 85, 76, 63, 55 };
  const float targetG[] = { 192, 157, 122, 111, 90, 78 };
  const float targetB[] = { 57, 47, 37, 36, 32, 29 };
...
void scanwifi_btn_event_cb(lv_event_t *e) {
  if (lv_event_get_code(e) == LV_EVENT_CLICKED) {
    ...
    // หน่วงให้ UI วาดก่อน แล้วค่อย scan
...
void KeyboardGardenName_event_cb(lv_event_t *e) {
    ...
    // เมื่อผู้ใช้กดปุ่มเครื่องหมาย "ติ๊กถูก" (ตกลง)
    if (code == LV_EVENT_READY) { 
...
void uiBattery() {
    ...
    // 1. แปลง ADC เป็นแรงดันที่ขา Pin (0 - 3.3V)
    // ใช้ 4095 เพราะความละเอียด 12-bit คือ 0 ถึง 4095
    float voltage = adcValue * (3.3 / 4095.0);
    
    // 2. คำนวณกลับเป็นแรงดันแบตเตอรี่จริง 
    // ถ้าคุณใช้ R สองตัวเท่ากันแบ่งครึ่ง แรงดันจริงต้องคูณ 2
    // สูตร: battVoltage = voltage * ((R1 + R2) / R2)
    float multiplier = 2.0; // ปรับตามอัตราส่วน R ที่คุณใช้จริง
    float battVoltage = voltage * multiplier;
    
    // 3. ปรับจูนค่า (Calibration) 
    // ADC ของ ESP32 มักจะมี Error ประมาณ 0.1V - 0.2V 
    // ให้ใช้มัลติมิเตอร์วัดเทียบแล้วบวกลบค่า Offset เอาครับ
    battVoltage += 0.12; 

    int percentage = 0;
    if (battVoltage >= 4.2) percentage = 100;
    else if (battVoltage >= 3.7) percentage = (battVoltage - 3.7) / (4.2 - 3.7) * 80 + 20;
    else if (battVoltage >= 3.2) percentage = (battVoltage - 3.2) / (3.7 - 3.2) * 20;
    else percentage = 0;

    p = constrain(percentage, 0, 100);;
...
void setup() {
  ...
  delay(1000);
  dfplayer.play(1); // ▶️ เล่นเพลงแรก (001.mp3)
...
const int interval = 5; // 5 ms

unsigned long lastSyncTime = 0;
const unsigned long syncInterval = 50000; // 120,000 ms = 2 นาที
...
void loop() {
  ...
  if (current_screen == ui_Capture) {
    ...
    updateGPS();
  } else if (current_screen == ui_Menu) {
    if (screen_changed && p <= 20) {
      dfplayer.play(3);
    }
    uiBattery();
    updateSyncStatusUI();
